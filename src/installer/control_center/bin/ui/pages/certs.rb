    ...
    stack @@installer_page_size_style do
      ...
      stack @@installer_content_size_style do
        ...
        stack :width => 1.0, :height => 40 do
          para ReadableNames["certs_page"]["heading"], @@heading_1_style 
        end

        stack :width => 1.0, :height => 100, :height => 60 do
          para ReadableNames["certs_page"]["help_note"], @@help_note_style
        end
        
        flow :width => 1.0, :height => 40 do
          stack :width => 250, :margin_left => 10, :margin_right => 10, 
              :margin_top => 20, :height => 45 do
             para ReadableNames["certs_page"]["trust_store_password"],  @@text_label_style
          end
          stack :width => 350, :margin_left => 10, :margin_right => 10, 
              :margin_top => 20, :height => 40 do
           @trust_store_password_edit = nl_text_edit( :short, text=$item.trust_store_password, 
        ...
        flow :width => 1.0, :height => 40 do
          stack :width => 250, :margin_left => 10, :margin_right => 10, 
              :margin_top => 20, :height => 45 do
             para ReadableNames["certs_page"]["confirm_password"],  @@text_label_style
          end
          stack :width => 350, :margin_left => 10, :margin_right => 10, 
              :margin_top => 20, :height => 40 do
           @trust_store_password_confi_edit = nl_text_edit( :short, text=$item.trust_store_password, 
               { :width => 200, :secret => true } )
          end
        end
      ...
      stack @@installer_footer_size_style do
        ...
        flow :width => 1.0, :height => 50 do
          ...
          stack :width => 50, :height => 1.0 do
            @ssl_cert_back_btn = nl_button :back
            @ssl_cert_back_btn.click { back_btn_click_proc.call }
          end

          stack :width => 50, :height => 1.0 do
            @ssl_cert_back_text_btn = nl_button :back_text, :click => back_btn_click_proc
          end
          ...
          next_btn_click_proc = Proc.new{
            ...
            control_fields = {
              "trust_store_password"    => @trust_store_password_edit
            }
            # First, the two passwords should match
            validated, field, error_msg = true, nil, nil

            if @trust_store_password_edit.text != @trust_store_password_confi_edit.text
              validated = false
              error_msg = ReadableNames["pop_up_messages"]["passwd_repeat_no_match"]
              field = "trust_store_password"
            end

            if validated
              # Then we can assign the value to item
              $item.trust_store_password = @trust_store_password_edit.text
              validated, field, error_msg = $item.validate_fields "trust_store_password"
            end
            if ! validated
          ...
          stack :width => 50, :height => 1.0 do
            @ssl_cert_next_text_btn = nl_button :next_text, :click => next_btn_click_proc
          end

          stack :width => 50, :height => 1.0 do
            @ssl_cert_next_btn = nl_button :next
            @ssl_cert_next_btn.click { next_btn_click_proc.call }
          end
