...
if [ "$1" == "-s" ];
then
    chmod +x $START_DIR/bin/suse_install.sh
    /bin/bash $START_DIR/bin/suse_install.sh
else
    chmod +x $START_DIR/bin/suse_install_ui.sh
    /bin/bash $START_DIR/bin/suse_install_ui.sh
fi