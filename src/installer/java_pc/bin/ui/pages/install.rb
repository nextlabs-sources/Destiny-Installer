...
class Installation < Shoes
  ...
  def install
    ...
    if $item.server_type.eql? "TOMCAT" then
      $envs.set :@CATALINA_HOME
    end
...
END_OF_CONFIG
    ...
    # chef_client_config_path = File.join(START_DIR, "bin", "config.rb")
    ...
    update_progress = lambda do |log_msg|
      ...
      case log_msg
      when /Pre Check - End/i then
        @progress.fraction = 0.6
      when /Pre Check/i then
        @progress_msg.text = strong(fg(ReadableNames["install_page"]["progress_precheck"],
            @@progress_msg_color_style))
        @progress.fraction = 0.5
      when /Install - Start/i then
        @progress_msg.text = strong(fg(ReadableNames["install_page"]["progress_install"],
            @@progress_msg_color_style))
        @progress.fraction = 0.65
      when /Install - End/i then
        @progress_msg.text = strong(fg(ReadableNames["install_page"]["progress_finish"],
            @@progress_success_msg_color_style))
    ...
    post_install_script_call = lambda do
      ...
      begin
        File.write(LOG_LOCATION, @install_util["original_log"])
...
#{Dir.tmpdir}/chef/bin/chef-client --config #{chef_client_config_path} \
-o PolicyController::main < nul
END_OF_SCRIPT
      
      @install_util["rollback_script"] = <<"END_OF_SCRIPT"
#{Dir.tmpdir}/chef/bin/chef-client --config #{chef_client_config_path} \
-o PolicyController::rollback_#{$item.server_type.downcase}
END_OF_SCRIPT
        
      @install_util["cleanup_backups_script"] = <<"END_OF_SCRIPT"
#{Dir.tmpdir}/chef/bin/chef-client --config #{chef_client_config_path} \
-o PolicyController::cleanup_backups_#{$item.server_type.downcase}
END_OF_SCRIPT
