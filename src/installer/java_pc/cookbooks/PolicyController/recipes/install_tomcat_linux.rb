...
# Recipe:: install_tomcat_linux
#     This handle the linux tomcat installer
#
...
ruby_block "copyJPCFiles" do
  ...
  notifies :run, 'bash[copyJPCFiles]', :immediately
end
...
# Giving file permission to nextlabs directory
bash 'nextlabs_dir_chmod_777' do
  destDir = node['installation_dir']
  code <<-EOH
    chmod -R 777 #{destDir}
  EOH
...
ruby_block "nextlabs_dir_permission" do
  block do
    destDir = node['installation_dir']
    puts "[Install]: Grant full permission for installation directory #{destDir}"
  end
  notifies :run, 'bash[nextlabs_dir_chmod_777]', :immediately
end
